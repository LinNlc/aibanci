(function(global){
  function create(utils){
    ...
    function buildWhiteFiveTwo({ employees, data, start, end, restPrefs }){
      ...
      for(const d of dates){
        ...
        for(const e of employees){
          ...
          row[e] = isRestDayForEmp(restPrefs, e, d) ? '休' : '白';
        }
        next[d] = row;
      }
      return next;
    ...
    function isRightEdgeWhite(data, day, emp){
      ...
      if(v !== '白') return false;
      const t1 = dateAdd(day, 1);
      const nxt = getVal(data, t1, emp);
      return nxt !== '白';
    }
    ...
    function isLeftEdgeMid(data, day, emp){
      ...
      const t0 = dateAdd(day, -1);
      const pre = getVal(data, t0, emp);
      return pre !== '中1';
    }
    ...
    function dailyMidCount(data, day, employees){
      const row = data[day] || {};
      let c = 0;
      for(const e of employees){ if(row[e] === '中1') c++; }
      return c;
    }
    ...
    function dailyWhiteCount(data, day, employees){
      const row = data[day] || {};
      let c = 0;
      for(const e of employees){ if(row[e] === '白') c++; }
      return c;
    }

    function empMidCountInRange(data, emp, start, end){
      let c = 0;
      for(const d of enumerateDates(start, end)){ if(getVal(data, d, emp) === '中1') c++; }
      return c;
    }

    function empWhiteCountInRange(data, emp, start, end){
      let c = 0;
      for(const d of enumerateDates(start, end)){ if(getVal(data, d, emp) === '白') c++; }
      return c;
    }
    ...
    function applyAlternateByCycle({ employees, data, start, end }, mixMaxRatio = 1){
      ...
      employees.forEach((e, idx)=>{
        ...
        cycles.forEach((cyc, ci)=>{
          ...
          if(shouldMid){
            for(const d of cyc){
              if(isRightEdgeWhite(next, d, e)){
                trySetVal(next, d, e, '中1', { start, end, mixMaxRatio });
              }
            }
          }
        });
        ...
        for(const d of days){
          ...
          if(M > high){
            ...
            cands = cands.sort((a,b)=>{
              const ratioA = empWhiteCountInRange(cur, b, start, end) > 0 ? empMidCountInRange(cur, b, start, end) / empWhiteCountInRange(cur, b, start, end) : 99;
              const ratioB = empWhiteCountInRange(cur, a, start, end) > 0 ? empMidCountInRange(cur, a, start, end) / empWhiteCountInRange(cur, a, start, end) : 99;
            ...
            cands = cands.sort((a,b)=>{
              const ratioA = empWhiteCountInRange(cur, a, start, end) > 0 ? empMidCountInRange(cur, a, start, end) / empWhiteCountInRange(cur, a, start, end) : 0;
              const ratioB = empWhiteCountInRange(cur, b, start, end) > 0 ? empMidCountInRange(cur, b, start, end) / empWhiteCountInRange(cur, b, start, end) : 0;
              return ratioA - ratioB;
            });
            for(const e of cands){
    ...
    function planAlbumSchedule({ albumDates, albumCheckedList, data, whiteHour, midHour, diffLimit }){
      const statsMap = {};
      ...
      const limit = Number.isFinite(diffLimit) ? Math.max(0, diffLimit) : Infinity;
      ...
      const chooseCandidate = (candidates, type, hourValue)=>{
        ...
        const feasible = sorted.filter(emp=>{
          ...
          return (maxVal - minVal) <= limit + 1e-9;
        });
        const pick = feasible.length ? feasible[0] : sorted[0];
        if(!feasible.length && limit !== Infinity) usedFallback = true;
      ...
      albumDates.forEach(day=>{
        ...
        const white = chooseCandidate(whiteCandidates, 'white', whiteHour);
        const mid = chooseCandidate(midCandidates, 'mid', midHour);
        plan[day] = { white: white || '', mid: mid || '' };
      });

      let note;
      if(missingCandidate){
        note = '部分日期缺少符合条件的白班或中1人员，请检查并手动补充。';
      } else if(usedFallback){
        note = '已尽量平均分配，但存在超出差值限制的情况，请复核。';
      } else {
        note = '自动生成完成，可继续手动微调，统计实时更新。';
      }

      return { plan, note, meta: { missingCandidate, usedFallback } };
    }
  ...
  global.SchedulerAlgorithms = { create };
})(window);
